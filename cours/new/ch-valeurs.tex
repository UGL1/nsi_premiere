...
Le type \pythoninline{int} dispose des opérations \pythoninline{+} (addition), \pythoninline{-} (soustraction) et \pythoninline{*} (multiplication).
...
On dispose également de \textit{deux opérations très pratiques} : soient \pythoninline{a} et \pythoninline{b} deux \pythoninline{int}, et \pythoninline{b} non nul, alors on 
\begin{itemize}
  \item \pythoninline{a // b} est le \textit{quotient} de la \textit{division euclidienne} de \mintinline{python}{a} par \mintinline{python}{b} ;  
...
\mintinline{python}{exp} est la \textit{fonction exponentielle} et \pythoninline{log} la \textit{fonction logarithme népérien}\footnote{Voir le programme de mathématiques
...
Il sert à représenter les \textit{chaînes de caractères} (\pythoninline{str} est l'abréviation de \textit{string}, qui veut dire chaîne en anglais).
Lorsqu'on écrit une valeur de type \pythoninline{str}, on peut utiliser les symboles \pythoninline{'}, \pythoninline{"} ou même \pythoninline{'''} (suivant que la chaîne contient des apostrophes, ou des guillemets).
...
Il sert à représenter les \textit{valeurs booléennes}, valant \pythoninline{True} (vrai) ou
...
Ce type dispose d'\textit{opérations logiques} : \pythoninline{or} (ou), \pythoninline{and} (et) et \pythoninline{not} (non).
